...
Example:
```bash
node dist/scripts/fetch_data.js --project-id PROJECT_ID
...
```
Run `node dist/scripts/validate.js --input {filename}` to check data format.
...
### Quality Check
1. Run validation script: `scripts/check_report.ts`
2. Identify issues:
    - Missing sections
