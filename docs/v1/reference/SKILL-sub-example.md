...
**所有认证操作只能通过本项目的 `node dist/scripts/cli.js` 完成，不得使用任何外部项目的工具：**

- **唯一执行方式**：只运行 `node dist/scripts/cli.js <子命令>`，不得使用其他任何实现方式。
...
```bash
node dist/scripts/cli.js list-accounts
```
...
- 所有 CLI 命令位于 `scripts/cli.ts`，输出 JSON。
...
```bash
node dist/scripts/cli.js check-login
```
...
```bash
node dist/scripts/cli.js wait-login
```
...
```bash
node dist/scripts/cli.js send-code --phone <用户确认的手机号>
```
...
```bash
node dist/scripts/cli.js verify-code --code <用户提供的6位验证码>
```
- 自动填写验证码、点击登录。
...
```bash
node dist/scripts/cli.js delete-cookies
node dist/scripts/cli.js --account work delete-cookies  # 指定账号
```
...
```bash
node dist/scripts/cli.js add-account --name work --description "工作号"
# 输出: {"success": true, "name": "work", "port": 9223, "profile_dir": "..."}

node dist/scripts/cli.js add-account --name personal
...
```bash
node dist/scripts/cli.js --account work check-login
node dist/scripts/cli.js --account work get-qrcode
node dist/scripts/cli.js --account personal check-login
node dist/scripts/cli.js check-login  # 不指定账号，使用默认端口 9222
```

### 管理账号

```bash
node dist/scripts/cli.js list-accounts                      # 列出所有账号及端口
node dist/scripts/cli.js set-default-account --name work    # 设置默认账号
node dist/scripts/cli.js remove-account --name personal     # 删除账号
```
